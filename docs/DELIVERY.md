...
**3D 建筑模型（追加）**：经用户提供阿里云百炼 `DASHSCOPE_API_KEY`、`WorkspaceId`（华北2北京）与腾讯云 `SecretId/SecretKey`，混用两家 3D 生成服务：**混元**（`tencentcloud-sdk-nodejs-ai3d`，高保真但单栋约 20MB）与 **Tripo**（[Tripo-3D](https://help.aliyun.com/zh/model-studio/tripo-3d-generation-api-reference)，`Tripo/Tripo-H3.1` 高保真 / `Tripo/Tripo-P1.0` 轻量低模）。最终 8 栋：潇湘馆/蘅芜苑用 **Tripo-H3.1**、怡红院/稻香村/秋爽斋用**混元**、栊翠庵/藕香榭/沁芳亭用 **Tripo-P1.0**。全部经 `gltf-transform` 优化压缩（draco + webp 贴图 + 简化 + 1024 贴图），单栋压缩后 <1.5MB（混元 20MB→约 0.3MB）。GLB 位于 `public/models/<id>.glb`（合计约 5MB）；`Garden.tsx` 经 `useGLTF` 加载自动缩放/落地，`src/ui/gardenModels.ts` 清单控制。旧混元原始模型备份至 `backup-models/hunyuan_backup/`（gitignore）。环境（树丛、草地、水面）已写实化。模型为单栋写实古建，比例/造型为艺术重构，非考证复原。注意：Tripo 有并发/日配额与 H3.1 需单独开通付费；混元按积分计费。
