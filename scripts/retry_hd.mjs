// 重跑剩余 H3.1 高保真生成+优化，带限流重试与任务间隔。
// 用法：node --env-file=.env scripts/retry_hd.mjs
...
async function submit(params) {
  ...
  if (!sj.output?.task_id) throw new Error(JSON.stringify(sj));
...
async function poll(spec, task) {
  for (let i = 0; i < 400; i++) {
    ...
    if (st === "SUCCEEDED") {
      ...
      writeFileSync(raw, Buffer.from(await (await fetch(g)).arrayBuffer()));
      const out = join(OUT, `${spec.id}.glb`);
      execFileSync("npx", ["gltf-transform", "optimize", raw, out,
        "--compress", "draco", "--texture-compress", "webp", "--texture-size", "1024", "--simplify-error", "0.001"], { stdio: "pipe" });
      const mb = (statSync(out).size / 1024 / 1024).toFixed(2);
      console.log(`  [${spec.id}] saved ${out} (${mb} MB)`);
      return true;
    }
    if (st === "FAILED" || st === "CANCELED" || st === "UNKNOWN") {
      const code = j.output?.code || "";
      console.log(`  [${spec.id}] TERMINAL ${code}`);
      return false;
    }
    await sleep(20000);
  }
...
async function genOne(spec) {
  ...
  for (let attempt = 0; attempt < 5; attempt++) {
    try {
      ...
      const ok = await poll(spec, task);
      return ok;
    } catch (e) {
      const msg = e.message;
      const transient = /InvalidApiKey|Throttling|RateQuota|rate limit/i.test(msg);
      console.log(`[${spec.id}] attempt ${attempt} err: ${msg.slice(0, 120)}`);
      if (!transient) return false;
      await sleep(45000 + attempt * 30000); // 退避
    }
  }
  return false;
}
...
async function main() {
  ...
  for (const s of SPECS) {
    ...
    if (r) ok++;
    await sleep(15000); // 任务间隔，避免连续提交被限流
  }
